...
import { getOrder, updateOrder as apiUpdate, deleteOrder as apiDelete } from '../api'
...
const DOC_FIELDS = [
  { key: 'docs_to_client_sent', label: 'Заявка клиенту' },
  { key: 'docs_from_client_received', label: 'Доки от клиента' },
  { key: 'docs_to_carrier_sent', label: 'Заявка перевозчику' },
  { key: 'docs_from_carrier_received', label: 'Доки от перевозчика' },
]
...
export default function OrderDetail({ orderId, onBack, onDelete }) {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 24, marginTop: 22, paddingTop: 18, borderTop: '1px solid rgba(255,255,255,0.1)' }}>
              ...
              <div>
                ...
                <div style={{ fontWeight: 800, fontSize: 20, fontFamily: 'Onest', color: '#1E9E5A' }}>
          ...
          <div className="card" style={{ padding: '20px 20px' }}>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              <button onClick={() => patch({ client_paid: !order.client_paid })} style={{
                flex: 1, padding: '14px 16px', borderRadius: 14, border: 'none', cursor: 'pointer',
                background: order.client_paid ? 'rgba(30,158,90,0.1)' : 'rgba(14,23,38,0.05)',
                outline: order.client_paid ? '2px solid rgba(30,158,90,0.4)' : '2px solid transparent',
                display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 4, transition: 'all 0.15s',
              }}>
                <div style={{ fontSize: 11, color: '#A6AEB8', fontWeight: 600 }}>ОТ КЛИЕНТА</div>
                <div style={{ fontFamily: 'Onest', fontWeight: 700, fontSize: 18, color: order.client_paid ? '#1E9E5A' : '#0E1726' }}>{fmtMoney(clientRate)}</div>
                <div style={{
                  padding: '2px 8px', borderRadius: 6, fontSize: 11, fontWeight: 600,
                  background: order.client_paid ? 'rgba(30,158,90,0.15)' : 'rgba(14,23,38,0.08)',
                  color: order.client_paid ? '#1E9E5A' : '#8A93A0',
                }}>{order.client_paid ? 'Оплачено' : 'Ожидается'}</div>
              </button>
              <button onClick={() => patch({ carrier_paid: !order.carrier_paid })} style={{
                flex: 1, padding: '14px 16px', borderRadius: 14, border: 'none', cursor: 'pointer',
                background: order.carrier_paid ? 'rgba(124,58,237,0.1)' : 'rgba(14,23,38,0.05)',
                outline: order.carrier_paid ? '2px solid rgba(124,58,237,0.4)' : '2px solid transparent',
                display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 4, transition: 'all 0.15s',
              }}>
                <div style={{ fontSize: 11, color: '#A6AEB8', fontWeight: 600 }}>ПЕРЕВОЗЧИКУ</div>
                <div style={{ fontFamily: 'Onest', fontWeight: 700, fontSize: 18, color: order.carrier_paid ? '#7C3AED' : '#0E1726' }}>{fmtMoney(carrierRate)}</div>
                <div style={{
                  padding: '2px 8px', borderRadius: 6, fontSize: 11, fontWeight: 600,
                  background: order.carrier_paid ? 'rgba(124,58,237,0.15)' : 'rgba(14,23,38,0.08)',
                  color: order.carrier_paid ? '#7C3AED' : '#8A93A0',
                }}>{order.carrier_paid ? 'Оплачено' : 'Ожидается'}</div>
              </button>
            </div>
          </div>
          ...
          {order.carrier_name && (
            <div className="card" style={{ padding: '20px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {order.carrier_plate && <Row label="Номер ТС" value={order.carrier_plate} />}
              </div>
            </div>
          )}
              ...
              {[
                ...
                <a
                  ...
                  style={{
                    ...
                    padding: '10px 12px', borderRadius: 12, background: doc.bg, cursor: doc.url ? 'pointer' : 'default',
